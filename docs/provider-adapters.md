...
| Antigravity / AGY OAuth | `sr agy` routed native launcher; server route `/antigravity` | Isolated server OAuth accounts imported with `sr agy add <label>` | `CLOUD_CODE_URL` points AGY at a short-lived local relay; relay replaces client auth with the selected router OAuth bearer | Plain `agy` | Server-side pooled selection/failover is per session and model-family aware; `--account` is a hard pin | Verified email and plan when exposed; independent Gemini and Claude/GPT quota families; ready/active/error | H1, H2, H3, H4 | Shadow proved two identities, project-bound replay, and server requests; real routed generation still requires a two-account Darwin 2xx canary |
