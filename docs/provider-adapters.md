...
| Antigravity / AGY OAuth | No supported native launcher in the current AGY release; future-compatible server route at `/antigravity` | Isolated profiles explicitly imported from the CLI's fixed Keychain slot with `sr agy add <label>` | Server adapter replaces client auth with the selected router OAuth bearer | Plain `agy` | Server adapter has pooled family-aware failover and hard pins; current `sr agy` fails closed because AGY's endpoint override changes request behavior | Verified email and plan when exposed; independent Gemini and Claude/GPT quota families; ready/active/error | H1, H2, H4; H3 proves launcher refusal | Two live OAuth identities, identity separation, quota/status, and server route tested; native AGY routed generation unsupported |
