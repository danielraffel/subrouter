...
// newLocalStoreAttestedClient returns a direct loopback HTTP client whose
...
func attestLocalStoreConnection(ctx context.Context, connection net.Conn, hostHeader string, store accounts.CodexStore) (net.Conn, error) {
	...
	return &bufferedAttestedConn{Conn: connection, reader: reader}, nil
}

type bufferedAttestedConn struct {
	net.Conn
	reader *bufio.Reader
}

func (c *bufferedAttestedConn) Read(buffer []byte) (int, error) {
